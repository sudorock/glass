(ns glass.timestamp
  ...
  (:import
   ...
   [java.time Instant LocalDate ZonedDateTime LocalDateTime ZoneId OffsetDateTime DateTimeException]
...
(extend-protocol Timestamp
  ...
  (->ts [s fmt]
    (let [formatter (DateTimeFormatter/ofPattern fmt)
      ...
      (if-let [parsed (try-parse fns)]
        (->ts parsed)
        (throw (Exception. ^String (format "Text %s could not be parsed" s))))))
  ...
  LocalDateTime
  (->ts [x]
    (->ts (.atZone x (ZoneId/of "UTC"))))

  LocalDate
  (->ts [x]
    (->ts (.atStartOfDay x (ZoneId/of "UTC")))))
